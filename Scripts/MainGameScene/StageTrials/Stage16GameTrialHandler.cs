...
public class Stage16GameTrialHandler : MonoBehaviour
{
    public AudioManager audioManager;
    public Animator clockAnimator;
    public GameObject MainStage16TrialPanel,SettingPanel;
    ...
    public void OpenStage16Trial()
    {
        StartCoroutine(OpenStage16TrialPanel());
        DisableDigitButtons();
    }
    private IEnumerator OpenStage16TrialPanel()
    {
        MainStage16TrialPanel.SetActive(true);
    ...
    private void GenerateQuestion()
    {
        ...
        while (!isValid)
        {
            int numOperands = Random.Range(2, 4);
            ...
            for (int i = 1; i < numOperands; i++)
            {
                if (operators[i - 1] == '/')
                {
                    operands[i] = Random.Range(1, 9); 
                    operands[i - 1] = operands[i] * Random.Range(1, 9); 
                }
            }
            List<int> nums = new List<int>(operands);
            List<char> ops = new List<char>(operators);
            ...
            if (correctAnswer >= 10 && correctAnswer <= 100)
            {
                ...
                if (questionText != null)
                {
                    ...
                    for (int i = 1; i < numOperands; i++)
                    {
                        questionText.text += " " + operators[i - 1] + " " + operands[i].ToString();
                    }
                    questionText.text += " =";
                }
            }
    ...
    private void EndGame(bool isVictory)
    {
        ...
        if (isVictory)
        {
            ...
            firebaseManager.UpdateStage16Task(true);
        }
        else
        {
    ...
    public void CloseMainStage16GameTrialPanel()
    {
        MainStage16TrialPanel.SetActive(false);
        DefeatPanel.SetActive(false);
        VictoryPanel.SetActive(false);
