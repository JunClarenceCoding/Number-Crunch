...
public class Stage12GameTrialHandler : MonoBehaviour
{
    public AudioManager audioManager;
    public Animator clockAnimator;
    public GameObject MainStage12TrialPanel,SettingPanel;
    ...
    public void OpenStage12Trial()
    {
        StartCoroutine(OpenStage12TrialPanel());
        DisableDigitButtons();
    }
    private IEnumerator OpenStage12TrialPanel()
    {
        MainStage12TrialPanel.SetActive(true);
    ...
    private void GenerateQuestion()
    {
        ...
        while (!isValid)
        {
            ...
            string[] operators = new string[numOperands - 1];
            operands[0] = Random.Range(2, 51); 
            correctAnswer = operands[0];
            for (int i = 1; i < numOperands; i++)
            {
                List<int> divisors = new List<int>();
                for (int j = 1; j <= correctAnswer; j++)
                {
                ...
                operands[i] = divisors[Random.Range(0, divisors.Count)];
                correctAnswer /= operands[i];
                operators[i - 1] = "/";
            }
            if (correctAnswer > 1 && correctAnswer <= 100)
            {
                isValid = true;
            }
            if (isValid && questionText != null)
            {
    ...
    private void EndGame(bool isVictory)
    {
        ...
        if (isVictory)
        {
            ...
            firebaseManager.UpdateStage12Task(true);
        }
        else
        {
    ...
    public void CloseMainStage12GameTrialPanel()
    {
        MainStage12TrialPanel.SetActive(false);
        DefeatPanel.SetActive(false);
        VictoryPanel.SetActive(false);
