...
public class Stage2BattleHandler : MonoBehaviour
{
    ...
    private void FetchMathSettings()
    {
        ...
        FirebaseDatabase.DefaultInstance.GetReference("MathSettings/Stage 2").GetValueAsync().ContinueWith(task =>
        {
    ...
    {
        // Check if the current text is just a negative sign, append the digit to form a negative number
        ...
        {
            // If the current text starts with a negative sign, handle negative number formation
        ...
        if (currentAnswer == correctAnswer)
        {
            ...
            else if (defensePhaseActive)
            {
                ...
                else
                {
                    ...
                    int excessDamage = correctAnswer - monsterDamage; // Calculate excess damage
    ...
    private void DeleteLastDigit()
    {
        if (currentAnswer != 0)
        {
            currentAnswer /= 10; // Remove the last digit by dividing by 10
            UpdateCurrentAnswerText(); // Update the UI to reflect the new value
            SetFeedback("", Color.clear); // Clear any feedback if necessary
        }
    }
    ...
    private void UpdateMonsterHealthText()
    {
        if (monsterHealthText != null && monsterComponent != null)
        {
            ...
            enemyHealthBar.SetHealth(monsterComponent.currentHealth); // Update the health bar
        }
    }
   ...
   private IEnumerator DelayedApplyDamage()
    ...
    IEnumerator LoadAsynchronously(string sceneName)
    {
        ...
        // Deactivate the loading screen once the scene is fully loaded
    ...
    private IEnumerator HandleVictorySequence(bool isStageCleared)
    {
        ...
        int xpReward = isStageCleared ? 50 : 350; 
        int coinReward = isStageCleared ? 30 : 250; 
    ...
    public void AddDamage()
    {
            ...
            {
                // Add the correct answer to accumulatedDefenseSum
                accumulatedDefenseSum += 175;

                // In defense phase, reduce monster damage
                if (175 <= monsterDamage)
                {
                    monsterDamage -= 175;
                    UpdatePhaseResultText(monsterDamage); // Update UI for monster damage
                }
                ...
                {
                    // If damage exceeds monster damage
                    int excessDamage = 175 - monsterDamage; // Calculate excess damage
                    totalDamage += excessDamage; // Add excess damage to totalDamage
                    monsterDamage = 0; // Monster damage reduced to 0
    ...
    public void RecordTotalAttempt()
    {
        ...
        // Directly specify the stage key ("stage02")
