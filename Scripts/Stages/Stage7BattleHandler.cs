...
public class Stage7BattleHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Get the instantiated player from BattleTrialCharacterLoader
    ...
    private void FetchMathSettings()
    {
        ...
        FirebaseDatabase.DefaultInstance.GetReference("MathSettings/Stage 7").GetValueAsync().ContinueWith(task =>
        {
    ...
    {
        int currentStage = 7; // Set this to your actual current stage
    ...
    private IEnumerator HandleVictorySequence(bool isStageCleared)
    {
        ...
        int xpReward = isStageCleared ? 65 : 480; 
        int coinReward = isStageCleared ? 35 : 300; 
    ...
    public void AddDamage()
    {
        ...
        {
            // Add the correct answer to accumulatedDefenseSum
            accumulatedDefenseSum += 185;

            // In defense phase, reduce monster damage
            if (185 <= monsterDamage)
            {
                monsterDamage -= 185;
                UpdatePhaseResultText(monsterDamage); // Update UI for monster damage
            }
            ...
            {
                // If damage exceeds monster damage
                int excessDamage = 185 - monsterDamage; // Calculate excess damage
                totalDamage += excessDamage; // Add excess damage to totalDamage
                monsterDamage = 0; // Monster damage reduced to 0
                UpdatePhaseResultText(monsterDamage); // Update UI for monster damage
                UpdateTotalDamageText(); // Update UI for total damage
    ...
    public void RecordTotalAttempt()
    {
        ...
        // Directly specify the stage key ("stage07")
