...
public class Stage15BattleHandler : MonoBehaviour
{
   ...
   void Start()
    ...
    private void FetchMathSettings()
    {
        ...
        int defaultRangeMax = 61;
        string defaultOperators = "/";
        int defaultNumOperandsMin = 2;
        int defaultNumOperandsMax = 3;

        // Fetching data from Firebase
        FirebaseDatabase.DefaultInstance.GetReference("MathSettings/Stage 15").GetValueAsync().ContinueWith(task =>
        {
    ...
    {
        int currentStage = 15; // Set this to your actual current stage
    ...
    private IEnumerator HandleVictorySequence(bool isStageCleared)
    {
        ...
        // Set rewards based on whether it's the first time clearing the stage
        int xpReward = isStageCleared ? 100 : 620; // 600 XP for first clear, 200 XP for subsequent clears
        int coinReward = isStageCleared ? 45 : 320; 
        ...
        if (!isStageCleared)
        {
            int gemsReward = 3; // Set the amount of Gems for the first-time clear reward
    ...
    public void AddDamage()
    {
        ...
        else if (defensePhaseActive)
        {
            ...
            {
                // If damage exceeds monster damage
                int excessDamage = 350 - monsterDamage; // Calculate excess damage
                totalDamage += excessDamage; // Add excess damage to totalDamage
                monsterDamage = 0; // Monster damage reduced to 0
                UpdatePhaseResultText(monsterDamage); // Update UI for monster damage
                UpdateTotalDamageText(); // Update UI for total damage
    ...
    public void RecordTotalAttempt()
    {
        ...
        // Directly specify the stage key ("stage15")
