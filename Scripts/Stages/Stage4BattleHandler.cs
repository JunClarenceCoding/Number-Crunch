...
public class Stage4BattleHandler : MonoBehaviour
{
    public Boss4Dialogue boss4Dialogue;
    ...
    public Button VictoryButton1,VictoryButton2;
    ...
    private void FetchMathSettings()
    {
        ...
        int defaultRangeMax = 21;
        string defaultOperators = "+";
        int defaultNumOperandsMin = 2;
        int defaultNumOperandsMax = 3;

        // Fetching data from Firebase
        FirebaseDatabase.DefaultInstance.GetReference("MathSettings/Stage 4").GetValueAsync().ContinueWith(task =>
        {
    ...
    {
        int currentStage = 4; // Set this to your actual current stage
    ...
    private IEnumerator HandleVictorySequence(bool isStageCleared)
    {
        ...
        int xpReward = isStageCleared ? 80 : 700; 
        int coinReward = isStageCleared ? 40 : 500; 
    ...
    public void AddDamage()
    {
        ...
        {
            // Add the correct answer to accumulatedDefenseSum
            accumulatedDefenseSum += 250;

            // In defense phase, reduce monster damage
            if (350 <= monsterDamage)
            {
                monsterDamage -= 250;
                UpdatePhaseResultText(monsterDamage); // Update UI for monster damage
            }
            ...
            {
                // If damage exceeds monster damage
                int excessDamage = 250 - monsterDamage; // Calculate excess damage
                totalDamage += excessDamage; // Add excess damage to totalDamage
                monsterDamage = 0; // Monster damage reduced to 0
                UpdatePhaseResultText(monsterDamage); // Update UI for monster damage
                UpdateTotalDamageText(); // Update UI for total damage
    ...
    {
        StartCoroutine(CheckStageCleared((isStageCleared) =>
        {
            if (isStageCleared)
            {
                onPressedStart();
            ...
            {
                boss4Dialogue.ConvoStart();
                ReadyPanel.SetActive(false);
            }
        }));
    }

    public void HandleStageBasedVictoryCheck()
    {
        StartCoroutine(CheckStageCleared((isStageCleared) =>
        {
            if (isStageCleared)
            ...
            {
                boss4Dialogue.StartVictoryDialogue();
            }
        }));
    }

    public void TriggerCheckStageClearedAndHandleVictory()
    {
        StartCoroutine(CheckStageClearedAndHandleVictory());
        UpdateStageProgress();
    }
    ...
    public void RecordTotalAttempt()
    {
        ...
        // Directly specify the stage key ("stage04")
