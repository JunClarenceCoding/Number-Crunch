...
public class Stage20BattleHandler : MonoBehaviour
{
    public Boss20Dialogue boss20Dialogue;
   ...
   void Start()
    ...
    private void FetchMathSettings()
    {
        ...
        int defaultRangeMax = 16;
        string defaultOperators = "+-*/";
        int defaultNumOperandsMin = 2;
        int defaultNumOperandsMax = 3;

        // Fetching data from Firebase
        FirebaseDatabase.DefaultInstance.GetReference("MathSettings/Stage 20").GetValueAsync().ContinueWith(task =>
        {
    ...
    {
        int currentStage = 20; // Set this to your actual current stage
        ...
        databaseReference.Child("users").Child(userId).Child("stages").Child("stage" + currentStageKey).Child("cleared").GetValueAsync().ContinueWith(task =>
        {
            ...
            else
            {
                ...
                if (currentStage + 1 <= 21) // Assuming stage 20 is the last stage
                {
    ...
    private IEnumerator HandleVictorySequence(bool isStageCleared)
    {
        ...
        int xpReward = isStageCleared ? 250 : 2000;
        int coinReward = isStageCleared ? 100 : 1000; 
         ...
         if (!isStageCleared)
        {
            ...
            {
                rewards.Add(new RewardItem("BoyClothes", 1)); 
            }
            else if (selectedCharacter == 1)
            {
                rewards.Add(new RewardItem("GirlClothes", 1));
            }
        }
    ...
    public void AddDamage()
    {
        ...
        {
            // Add the correct answer to accumulatedDefenseSum
            accumulatedDefenseSum += 700;

            // In defense phase, reduce monster damage
            if (700 <= monsterDamage)
            {
                monsterDamage -= 700;
                UpdatePhaseResultText(monsterDamage); // Update UI for monster damage
            }
            ...
            {
                // If damage exceeds monster damage
                int excessDamage = 700 - monsterDamage; // Calculate excess damage
                totalDamage += excessDamage; // Add excess damage to totalDamage
                monsterDamage = 0; // Monster damage reduced to 0
                UpdatePhaseResultText(monsterDamage); // Update UI for monster damage
                UpdateTotalDamageText(); // Update UI for total damage
    ...
    public void RecordTotalAttempt()
    {
        ...
        // Directly specify the stage key ("stage20")
